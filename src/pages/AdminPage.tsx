...
import {
    ...
    useUpdateSection, useUpdateVideo, useUpdateVideoOrder, useUpdateCourse, useGetSettings, useUpdateSettings, useAffiliateLeaderboard,type AdminCourse , useGetMembershipPrices, useUpdateMembershipPrices,type PricingGrid, useUpdateSectionOrder
} from '../hooks/useAdmin';
...
const AdminCourseDetailView: FC<{ courseId: string, onBack: () => void }> = ({ courseId, onBack }) => {
    const { t } = useTranslation();
                                                                    ...
                                                                    <Draggable key={video.id} draggableId={video.id} index={vIndex}>
                                                                        {(provided) => (
                                                                            <li ref={provided.innerRef} {...provided.draggableProps} className="group flex items-center gap-4 p-3 bg-[#111317] rounded-lg">
                                                                                <div {...provided.dragHandleProps} className="text-neutral-500 cursor-grab p-2 -ml-2">
                                                                                    <FaDragHandle />
                                                                                </div>
